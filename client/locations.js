Template.countrySelect.onCreated(function() {
    this.countries = [{
        "name": "United States",
        "code": "US",
    ...
    Session.set('country-filename','united-states'); // default to US because MURICA
});

Template.countrySelect.events({
    'change #country-select': function(event, template) {
        var filename = $(event.currentTarget).find(':selected').attr('data-filename');
        Session.set('country-filename', filename);
        }
});
...
Template.stateProvinceSelect.onCreated(function() {
    var self = this;
    self.provinces = new ReactiveVar([]);

    Tracker.autorun(function() {
        var countryFile = Session.get('country-filename');

        if (!countryFile) {
            // this country has no states or provinces
            self.provinces.set([]);
            return;
        }

        Meteor.call('loadCountryFile', countryFile, function(error, response) {
            var provinceArray = [];
            ...
            self.provinces.set(provinceArray);
            $("#state-province-select")[0].selectedIndex = 0;
        });

    });
});

Template.stateProvinceSelect.helpers({
    'provinces': function() {
        return Template.instance().provinces.get();
    }
});
